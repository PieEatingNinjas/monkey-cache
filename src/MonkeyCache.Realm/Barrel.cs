...
namespace MonkeyCache.Realm
{
	public class Barrel : IBarrel
	{
		...
		Barrel()
		{
			...
			db = Realms.Realm.GetInstance(config);
		...
		public DateTime? GetExpiration(string key)
		{
			...
			return ent.ExpirationDate.UtcDateTime;
		}

		private void InRealmTransaction(Realms.Realm database, Action<Realms.Realm> action)
		{
		...
		void Add(string key, string data, TimeSpan expireIn, string eTag = null)
		{
			...
			db.Write(() =>
			{
				...
				{
					ent.ExpirationDate = Utils.GetExpiration(expireIn);
					ent.ETag = eTag;
					ent.Contents = data;
				}
			});
		...
		public void EmptyExpired()
			=> InRealmTransaction(db, d => d.RemoveRange(d.All<Banana>().Where(b => b.ExpirationDate.ToUniversalTime() < DateTime.UtcNow)));
		...
		public void EmptyAll()
			=> InRealmTransaction(db, d => d.RemoveAll(nameof(Banana)));
	...
	internal static class RealmMonkeyCacheExtensions
	{
		 internal static Banana FindBanana(this Realms.Realm realm, string key)
			=> realm.Find<Banana>(key);
	}
}
